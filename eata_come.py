...
class EATA_COME(nn.Module):
    """EATA adapts a model by entropy minimization during testing.
    Once EATAed, a model adapts itself by updating on every forward.
    """
    def __init__(self, model, optimizer, fishers=None, fisher_beta=2000.0, steps=1, episodic=False, e_margin=math.log(1000)/2-1, d_margin=0.05):
        super().__init__()
        self.model = model
        ...
        self.fisher_beta = fisher_beta # trade-off \beta for two losses (Eqn. 8) 
    ...
    def forward(self, x):
        ...
        if self.steps > 0:
            for _ in range(self.steps):
                outputs, num_counts_2, num_counts_1, updated_probs = forward_and_adapt_eata(x, self.model, self.optimizer, self.fishers, self.e_margin, self.current_model_probs, fisher_beta=self.fisher_beta, num_samples_update=self.num_samples_update_2, d_margin=self.d_margin)
...
def softmax_entropy(x: torch.Tensor) -> torch.Tensor:
    ...
    x = torch.div(x, norms) * norms.clone().detach()
    x = -(x.softmax(1) * x.log_softmax(1)).sum(1)
    return x

@torch.jit.script
def dirichlet_entropy(x):#key component of COME
...
def forward_and_adapt_eata(x, model, optimizer, fishers, e_margin, current_model_probs, fisher_beta=50.0, d_margin=0.05, scale_factor=2, num_samples_update=0):
    ...
    if fishers is not None:
        ...
        for name, param in model.named_parameters():
            if name in fishers:
                ewc_loss += fisher_beta * (fishers[name][0] * (param - fishers[name][1])**2).sum()
        loss += ewc_loss
    if x[ids1][ids2].size(0) != 0:
        loss.backward()
        optimizer.step()
